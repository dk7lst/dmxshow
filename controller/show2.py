...
def DomeEffects():
  r = g = b = white = amber = uv = 0
  for i in range(0, 3):
    colorMode = globalEffect = random.randint(0, 5)
    if colorMode == 0: r = random.randint(100, 255)
    elif colorMode == 1: g = random.randint(100, 255)
    elif colorMode == 2: b = random.randint(100, 255)
    elif colorMode == 3: white = random.randint(100, 255)
    elif colorMode == 4: amber = random.randint(100, 255)
    elif colorMode == 5: uv = random.randint(100, 255)
  DomeEffects.direction = not DomeEffects.direction
  motor = 6 if DomeEffects.direction else 127
  dmx.clear() # solo effect
  dome.set(r, g, b, white, amber, uv, 0, motor)
DomeEffects.direction = False

def GlobalEffectsThread():
  global globalEffect, sync
  while True:
    sync.waitBeat()
    if random.randint(0, 100) < 5:
      globalEffect = random.randint(3, 9)
      if globalEffect != 9: dome.clear() # Switch off motor when not in use
...
sync = MidiSync.MidiSync(True)
#print(sync.getAvailablePorts())
sync.openPort(0)

dmx = ENTTEC_ESP.ENTTEC_ESP("192.168.4.173")
...
dome = Fixtures.FunGeneration_LED_Diamond_Dome_MK_II(dmx, 172)
# Next free address: 181
...
while True:
  ...
  dmx.flush()
  time.sleep(1.0 / 40) # Run at around 40 fps
